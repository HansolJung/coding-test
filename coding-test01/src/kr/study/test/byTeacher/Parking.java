	...
	public static int[] solution(int[] fees, String[] records) {
		...
		for (String record : records) {
			...
			if (!carMap.containsKey(carInfo[1])) {
				...
				carMap.remove(carInfo[1]); // 출차된 차 정보를 맵에서 삭제
				
			}
		}
		
		// 입차 후 출차가 안된 경우 > 23:59 에 출차했다고 가정하고 계산
		...
		int i = 0; // answer 의 index 처리용 변수
		
		// carTimeMap 은 TreeMap 이기 때문에 정렬할 필요 없음
		for (String key : carTimeMap.keySet()) {
			int stayTime = carTimeMap.get(key);
			
			// 주차 시간이 기본 시간을 넘지 않으면 기본요금
			if (stayTime <= fees[0]) {
				answer[i++] = fees[1];
				continue;
			} 
